...
-- =================================================================
CREATE OR REPLACE FUNCTION public.delete_exam_registration(registration_id uuid)
RETURNS json AS $$
DECLARE
    plata_record RECORD;
...
BEGIN
    ...
    SELECT * INTO inscriere_record FROM public.inscrieri_examene WHERE id = registration_id;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Inscrierea cu ID-ul % nu a fost găsită.', registration_id;
    END IF;

    -- Find the associated payment using the reference in 'observatii'
    SELECT * INTO plata_record FROM public.plati WHERE observatii LIKE '%Ref Inscriere: ' || registration_id || '%';
    ...
    IF FOUND THEN
        ...
        DELETE FROM public.plati WHERE id = plata_record.id;
    END IF;

    -- Delete the main registration record
    DELETE FROM public.inscrieri_examene WHERE id = registration_id;

    RETURN json_build_object(
        'success', true,
        'message', 'Înscrierea a fost ștearsă cu succes.',
        'deleted_plata_id', plata_record.id -- can be null
    );
END;
