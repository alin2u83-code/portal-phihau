...
const ExamenDetail: React.FC<ExamenDetailProps> = ({ examen, participari, setParticipari, sportivi, grade, setPlati, preturi, allParticipari, examene }) => {
    ...
    const handleAddParticipant = async (e: React.FormEvent) => {
        ...
        if (nextGrad) {
            const ageAtExam = getAgeOnDate(sportiv.dataNasterii, examen.data); const lastExamDate = admittedParticipations.length > 0 ? new Date(examene.find(ex => ex.id === admittedParticipations[0].examenId)!.data) : new Date(sportiv.dataInscrierii); const monthsToWait = parseDurationToMonths(nextGrad.timpAsteptare); const eligibilityDate = new Date(lastExamDate); eligibilityDate.setMonth(eligibilityDate.getMonth() + monthsToWait);
            if (ageAtExam >= nextGrad.varstaMinima && new Date(examen.data) >= eligibilityDate) gradSustinut = nextGrad; else confirmationOnly = true;
        }
        
        const {data: participareData, error: pError} = await supabase.from('participari').insert({ examenId: examen.id, sportivId: sportivId, gradSustinutId: gradSustinut.id, rezultat: 'Neprezentat' }).select().single();
        ...
        const {data: plataData, error: plError} = await supabase.from('plati').insert({ sportivId, familieId: sportiv.familieId, suma: pretExamenConfig.suma, data: examen.data, status: 'Neachitat', descriere, tip: 'Taxa Examen', observatii: '' }).select().single();
...
export const ExameneManagement: React.FC<ExameneManagementProps> = ({ onBack, examene, setExamene, participari, setParticipari, sportivi, grade, setPlati, preturi }) => {
  ...
  const handleDelete = async (examenId: string) => {
    ...
    const { error: participariError } = await supabase.from('participari').delete().eq('examenId', examenId);
    if(participariError) { alert(`Eroare la ștergerea participarilor: ${participariError.message}`); return; }
    setParticipari(prev => prev.filter(p => p.examenId !== examenId));
