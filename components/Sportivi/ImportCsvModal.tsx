...
export const ImportCsvModal: React.FC<ImportCsvModalProps> = ({ isOpen, onClose, onImportComplete, activeClubId, defaultGrupaId, grade }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <Card className="w-full max-w-2xl bg-zinc-900 border-zinc-800 p-6 shadow-2xl">
                    ...
                    {(['upload', 'mapping', 'preview', 'result'] as Step[]).map((s, i) => {
                        ...
                        return (
                            <React.Fragment key={s}>
                                <span className={`px-2 py-0.5 rounded ${active ? 'bg-amber-500 text-black font-semibold' : done ? 'text-green-400' : 'text-zinc-500'}`}>
                                    {labels[s]}
                                </span>
                                {i < 3 && <ChevronRight className="w-3 h-3 text-zinc-600" />}
                            </React.Fragment>
                        );
                    })}
                </div>
                ...
                <div className="mt-5 flex justify-between items-center border-t border-zinc-800 pt-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        {step === 'result' && status === 'success' && (
                            <Button onClick={onImportComplete}>Finalizează</Button>
                        )}
                    </div>
                </div>
            </Card>
