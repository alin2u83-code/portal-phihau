...
export const Input: React.FC<InputProps> = ({ label, id, ...props }) => (
    <div>
        ...
        <input id={id} {...props} className={`w-full bg-slate-900/50 border border-slate-600 rounded-md px-2 py-1 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-secondary focus:border-brand-secondary focus:bg-slate-800 disabled:opacity-50 disabled:bg-slate-700 ${props.className}`} />
    </div>
);
...
export const Select: React.FC<SelectProps> = ({ label, id, children, ...props }) => (
    <div>
        {label && <label htmlFor={id} className="block text-sm font-medium text-slate-300 mb-1">{label}</label>}
        <select id={id} {...props} className={`w-full bg-slate-900/50 border border-slate-600 rounded-md px-2 py-1 text-white focus:outline-none focus:ring-2 focus:ring-brand-secondary focus:border-brand-secondary focus:bg-slate-800 disabled:opacity-50 disabled:bg-slate-700 ${props.className}`}>
            {children}
        </select>
    </div>
);