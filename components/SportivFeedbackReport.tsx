...
export const SportivFeedbackReport: React.FC<SportivFeedbackReportProps> = ({ isOpen, onClose, sportiv, antrenamente, grupe, grade, participari, examene }) => {
    ...
    const reportData = useMemo(() => {
        ...
        const admitted = participari.filter(p => p.sportiv_id === sportiv.id && p.rezultat === 'Admis').sort((a,b) => new Date(examene.find(e=>e.id===b.examen_id)!.data).getTime() - new Date(examene.find(e=>e.id===a.examen_id)!.data).getTime());
        ...
        if (nextGrad) {
            const lastExamDate = admitted[0] ? new Date(examene.find(e=>e.id===admitted[0].examen_id)!.data) : new Date(sportiv.data_inscrierii);
