...
const EvenimentDetail: React.FC<EvenimentDetailProps> = ({ eveniment, rezultate, setRezultate, sportivi, setPlati, preturiConfig }) => {
    ...
    const handleAddParticipant = (e: React.FormEvent) => {
        ...
        setRezultate(prev => [...prev, { id: new Date().toISOString(), eveniment_id: eveniment.id, sportiv_id: sportivId, rezultat: 'Înscris', probe: '' }]);
        ...
        const newPlata: Plata = { id: new Date().toISOString(), sportiv_id: sportiv.id, familie_id: sportiv.familie_id, suma: pretConfig.suma, data: new Date().toISOString().split('T')[0], status: 'Neachitat', descriere: `Taxa ${eveniment.denumire}`, tip: categorie, metoda_plata: null, data_platii: null, observatii: `Generată la înscrierea în eveniment.`, };
...
export const StagiiCompetitiiManagement: React.FC<StagiiCompetitiiProps> = ({ onBack, type, evenimente, setEvenimente, rezultate, setRezultate, sportivi, setPlati, preturiConfig }) => {
    ...
    const handleDelete = (evId: string) => { if (window.confirm("Ești sigur? Toate rezultatele asociate vor fi șterse.")) { setEvenimente(prev => prev.filter(e => e.id !== evId)); setRezultate(prev => prev.filter(p => p.eveniment_id !== evId)); if (selectedEv?.id === evId) setSelectedEv(null); } };
