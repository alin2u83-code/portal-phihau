...
const GradFormModal: React.FC<{ isOpen: boolean; onClose: () => void; onSave: (grad: Omit<Grad, 'id'>) => Promise<void>; grade: Grad[]; gradToEdit: Grad | null; }> = ({ isOpen, onClose, onSave, grade, gradToEdit }) => {
  const [formState, setFormState] = useState<Omit<Grad, 'id'>>(emptyFormState);
  ...
  const handleSubmit = async (e: React.FormEvent) => { 
    ...
    if (!formState.nume || formState.ordine <= 0) { 
      alert("Numele și ordinea (pozitivă) sunt obligatorii."); 
      return; 
    } 
    setLoading(true); 
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={gradToEdit ? "Editează Grad" : "Adaugă Grad Nou"}> 
      <form onSubmit={handleSubmit} className="space-y-4"> 
        ...
        <div className="flex justify-end pt-4 space-x-2">
          ...
          <Button variant="success" type="submit" disabled={loading}>{loading ? 'Se salvează...' : 'Salvează'}</Button>
        </div> 
      </form> 
    </Modal> 
  );
...
export const GradeManagement: React.FC<{ grade: Grad[]; setGrade: React.Dispatch<React.SetStateAction<Grad[]>>; onBack: () => void; preturiConfig: PretConfig[]; setPreturiConfig: React.Dispatch<React.SetStateAction<PretConfig[]>>; }> = ({ grade, setGrade, onBack, preturiConfig, setPreturiConfig }) => {
  ...
  const handlePriceUpdate = async (grad: Grad, newPriceStr: string) => { 
      ...
      if (error) { 
        showError("Eroare la salvare preț", error); 
      } else if (data) { 
  ...
  const handleSaveGrad = async (gradData: Omit<Grad, 'id'>) => { 
    ...
    if (gradToEdit) { 
      ...
      if (error) { 
        ...
        setGrade(prev => prev.map(g => g.id === gradToEdit.id ? data as Grad : g)); 
      } 
    } else { 
      ...
      if (error) { 
        ...
        setGrade(prev => [...prev, data as Grad]); 
      } 
    } 
  };
  ...
  const confirmDelete = async () => { 
    ...
    if (error) { 
      ...
      setGrade(prev => prev.filter(g => g.id !== gradToDelete.id)); 
    } 
