...
export const ImportSportiviPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    // false = completeazÄƒ doar cÃ¢mpurile lipsÄƒ (default sigur), true = suprascrie tot cu datele din CSV
    ...
    const handleExecuteImport = async () => {
        ...
        // CÃ¢nd overwriteMode=false, excludem din update cÃ¢mpurile care existÄƒ deja Ã®n DB
        const buildUpdatePayload = (sportivData: any, existingSportiv: any) => {
            ...
            for (const key of Object.keys(sportivData)) {
                if (key === 'id') continue;
                // PÄƒstrÄƒm cÃ¢mpul din CSV doar dacÄƒ Ã®n DB este null/undefined/gol
    ...
    const buildUnifiedRows = (): UnifiedRow[] => {
        ...
        toImportList.forEach(s => {
            rows.push({
                ...
                nume: s.nume,
                prenume: s.prenume,
                dataNasteriiCSV: s.rawDate || s.data_nasterii || 'â€”',
        ...
        strictDuplicates.forEach((d, i) => {
            rows.push({
                ...
                dataNasteriiCSV: d.rawDate || d.sportivData.data_nasterii || 'â€”',
        ...
        looseDuplicates.forEach((d, i) => {
            ...
            if (dataNasteriiNorm !== d.existingSportiv.data_nasterii) {
                const csvDateDisplay = d.rawDate || 'â€”';
                ...
                <div className="space-y-3">
                    ...
                    <div className="border border-zinc-800 rounded-lg overflow-hidden">
                        ...
                        >
                            ...
                            <span className="text-zinc-400 text-sm">{expandedSections.has('adaugati') ? 'â–²' : 'â–¶'}</span>
                        </button>
                    ...
                    <div className="border border-zinc-800 rounded-lg overflow-hidden">
                        ...
                        >
                            ...
                            <span className="text-zinc-400 text-sm">{expandedSections.has('actualizati') ? 'â–²' : 'â–¶'}</span>
                        </button>
                    ...
                    <div className="border border-zinc-800 rounded-lg overflow-hidden">
                        ...
                        >
                            ...
                            <span className="text-zinc-400 text-sm">{expandedSections.has('omisi') ? 'â–²' : 'â–¶'}</span>
                        </button>
                            ...
                            <div className="max-h-60 overflow-y-auto divide-y divide-zinc-800/50">
                                ...
                                {importResult.omisi.map((s, i) => (
                                    <div key={i} className="px-4 py-2 text-sm">
                                        ...
                                        <span className="text-red-400 ml-2">â€” {s.motiv}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                                ...
                                {unifiedRows.map((row, idx) => (
                                    <React.Fragment key={idx}>
                                        <tr className={`hover:bg-zinc-800/30 transition-colors ${row.status === 'EROARE' ? 'opacity-60' : ''}`}>
                                            ...
                                            <td className="px-3 py-2.5 text-zinc-400 tabular-nums">{row.dataNasteriiCSV || 'â€”'}</td>
                                            <td className="px-3 py-2.5">{getStatusBadge(row.status)}</td>
                                            <td className="px-3 py-2.5 text-zinc-500 text-xs max-w-xs truncate" title={row.motiv}>{row.motiv || 'â€”'}</td>
                                            <td className="px-3 py-2.5">
                                                {row.status === 'POSIBIL_DUPLICAT' && row.looseIndex !== undefined && (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        >
                                                            {expandedRows.has(row.originalIndex) ? 'â–² Ascunde' : 'â–¼ Detalii'}
                                                        </button>
                                                    </div>
                                                )}
                                                        ...
                                                        <table className="w-full text-xs">
                                                            ...
                                                            <tbody>
                                                                <tr className="border-t border-zinc-700">
                                                                    ...
                                                                    <td className="px-3 py-1.5 text-zinc-200">{row.sportivData?.nume || 'â€”'}</td>
                                                                    <td className="px-3 py-1.5 text-zinc-200">{row.sportivData?.prenume || 'â€”'}</td>
                                                                    <td className="px-3 py-1.5 text-zinc-200">{row.dataNasteriiCSV || 'â€”'}</td>
                                                                    <td className="px-3 py-1.5 text-zinc-400">{row.sportivData?.cnp || 'â€”'}</td>
                                                                </tr>
                                                                <tr className="border-t border-zinc-700">
                                                                    ...
                                                                    <td className="px-3 py-1.5 text-zinc-400">{row.existingSportiv.cnp || 'â€”'}</td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                                    </div>
                        ...
                        {unifiedRows.map((row, idx) => (
                            <div key={idx} className={`p-3 ${row.status === 'EROARE' ? 'opacity-60' : ''}`}>
                                ...
                                <div className="text-xs text-zinc-500 mb-1.5">
                                    Data: <span className="text-zinc-400">{row.dataNasteriiCSV || 'â€”'}</span>
                                </div>
                                {row.motiv && (
                                ...
                                {row.status === 'POSIBIL_DUPLICAT' && expandedRows.has(row.originalIndex) && row.existingSportiv && (
                                    <div className="mt-2 rounded border border-zinc-700 overflow-hidden text-xs">
                                        ...
                                        <div className="grid grid-cols-[3rem_1fr_1fr_1fr] border-t border-zinc-700">
                                            ...
                                            <div className="px-2 py-1.5 text-zinc-200">{row.sportivData?.nume || 'â€”'}</div>
                                            <div className="px-2 py-1.5 text-zinc-200">{row.sportivData?.prenume || 'â€”'}</div>
                                            <div className="px-2 py-1.5 text-zinc-200">{row.dataNasteriiCSV || 'â€”'}</div>
                                        </div>
                                        <div className="grid grid-cols-[3rem_1fr_1fr_1fr] border-t border-zinc-700">
                ...
                <div className="flex items-start gap-3 p-3 bg-zinc-800/50 border border-zinc-700 rounded-lg">
                    ...
                    <div>
                        <p className="text-sm font-medium text-zinc-200">
                            {overwriteMode ? 'Suprascrie cÃ¢mpurile existente' : 'CompleteazÄƒ doar cÃ¢mpurile lipsÄƒ'}
                        </p>
                        <p className="text-xs text-zinc-500 mt-0.5">
                            {overwriteMode
                                ? 'Datele din CSV vor Ã®nlocui cÃ¢mpurile existente Ã®n baza de date, chiar dacÄƒ au deja valori.'
                                : 'Implicit: cÃ¢mpurile care au deja valori Ã®n baza de date nu vor fi modificate.'}
                        </p>
                        {overwriteMode && (
                            <p className="text-xs text-amber-400 mt-1 flex items-center gap-1">
                                <span>âš </span> AtenÈ›ie: aceastÄƒ opÈ›iune poate suprascrie date corecte existente.
                            </p>
                        )}
                    </div>
                </div>
