    ...
    const sidebarContent = (
        <div className="flex flex-col h-full bg-[var(--bg-card)] text-white shadow-xl">
            ...
            <nav className="flex-1 px-2 py-4 space-y-1.5 overflow-y-auto">
                ...
                <div onClick={() => handleNavigate('debug')} className={`flex items-center p-2.5 text-white rounded-md cursor-pointer transition-colors w-full ${'debug' === activeView ? "bg-[var(--accent)] text-white shadow-lg" : "hover:bg-white/10"}`} title={!isExpanded ? 'Debug' : ''}>
