...
const NavItem: React.FC<{ item: any; isExpanded: boolean; onClick?: () => void }> = ({ item, isExpanded, onClick }) => {
    ...
    const linkContent = (
        <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${isActive ? 'bg-brand-primary/10 text-brand-primary border border-brand-primary/20' : 'hover:bg-slate-800 text-slate-400 hover:text-slate-200'}`}>
...
export const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const SidebarContent: React.FC<{isExpanded: boolean}> = ({ isExpanded }) => (
        <div className="flex flex-col h-full bg-brand-card">
             <div className="p-4 border-b border-slate-700 h-16 flex items-center gap-3">
                ...
                {isExpanded && <span className="font-bold text-xl tracking-tight text-white">Phi Hau</span>}
            </div>
            <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto">
                {menuItems.map(item => <NavItem key={item.label} item={item} isExpanded={isExpanded} onClick={() => setIsMobileMenuOpen(false)} />)}
            </nav>
            <div className="p-3 border-t border-slate-700 space-y-1">
                <button className="w-full flex items-center gap-3 px-3 py-2.5 text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-lg transition-all" onClick={() => window.location.reload()}>
                    ...
                    {isExpanded && <span className="font-medium text-sm">Sincronizare</span>}
                </button>
                <button onClick={() => logout()} className="w-full flex items-center gap-3 px-3 py-2.5 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-all">
                    ...
                    {isExpanded && <span className="font-medium text-sm">Deconectare</span>}
                </button>
            </div>
        </div>
    );
    ...
    return (
        <div className="flex h-screen bg-brand-dark text-slate-200 overflow-hidden">
            {/* Desktop Sidebar */}
            <aside className={`hidden lg:flex flex-col border-r border-slate-700 transition-all duration-300 ${isSidebarExpanded ? 'w-64' : 'w-20'}`}>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-slate-700 flex flex-col lg:hidden transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <SidebarContent isExpanded={true} />
            </aside>

            <div className="flex-1 flex flex-col overflow-hidden">
                <header className="h-16 bg-brand-card/50 backdrop-blur-sm border-b border-slate-700 flex items-center justify-between px-4 md:px-6">
                    <div className="flex items-center gap-2">
                        <button onClick={() => setIsSidebarExpanded(!isSidebarExpanded)} className="hidden lg:block p-2 rounded-full hover:bg-slate-700/50"><Menu className="w-5 h-5" /></button>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-brand-primary to-blue-600 flex items-center justify-center font-bold text-white shadow-lg">
