...
export const Pas1Revizuire: React.FC<Props> = ({
    ...
    const countOmis = unifiedRows.filter(r => r.status === 'OMIS').length;

    return (
        <Card className="p-4 md:p-6">
            <h2 className="text-xl md:text-2xl font-bold mb-3 text-zinc-100">Revizuire Import</h2>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
                {[
                    ...
                    countOmis > 0 && { key: 'OMIS', label: `Omiși (${countOmis})`, cls: 'bg-slate-600/40 text-slate-400 border-slate-600' },
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                ...
                {countOmis > 0 && (
                    <span className="bg-slate-500/20 text-slate-400 border border-slate-500/50 px-3 py-1 rounded text-sm font-medium">
                        OMIȘI: {countOmis}
                    </span>
                )}
            </div>
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            {visibleRows.map((row, idx) => (
                                <React.Fragment key={idx}>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {row.fieldComparisons && row.fieldComparisons.length > 0 && (
                                                        <button
                                                            ...
                                                            onClick={() => onToggleExpandRow(row.originalIndex)}
                                                        >
                                                            {expandedRows.has(row.originalIndex) ? '▲ Ascunde' : '▼ Câmpuri'}
                                                        </button>
                                                    )}
                                                </div>
                                            )}
                                    ...
                                    {/* Tabel comparativ vechi pentru POSIBIL_DUPLICAT (fără fieldComparisons) */}
                                    {row.status === 'POSIBIL_DUPLICAT' && expandedRows.has(row.originalIndex) && row.existingSportiv && !row.fieldComparisons?.length && (
                                        <tr>
