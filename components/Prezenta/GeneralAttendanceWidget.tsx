...
const LoadingSpinner: React.FC = () => (
    <div className="flex-grow flex flex-col items-center justify-center">
        ...
        <p className="text-sm text-slate-400 mt-2">Se Ã®ncarcÄƒ sumarul...</p>
    </div>
);
        ...
        const fetchTodayStats = async () => {
            ...
            try {
                ...
                if (error.code === '42501' || error.message.includes('permission denied')) {
                    ...
                    showError('Eroare widget prezenÈ›Äƒ', error.message);
                }
            } finally {
                setLoading(false);
            }
    ...
    return (
        <Card className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 p-6 flex flex-col justify-center text-center h-full">
            <div className="flex items-center justify-center gap-2">
                ...
                <h3 className="text-xl font-bold text-white">PrezenÈ›a GeneralÄƒ Azi</h3>
            </div>
            
            {loading ? <LoadingSpinner/> : stats.trainingsCount === 0 ? (
                <div className="flex-grow flex items-center justify-center">
                    <p className="text-slate-400 italic">Niciun antrenament programat astÄƒzi.</p>
                </div>
            ) : (
                <div className="mt-4">
                    ...
                    <p className="text-slate-300 text-base">Sportivi PrezenÈ›i</p>
