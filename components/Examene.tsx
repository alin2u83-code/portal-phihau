...
const ExamenDetail: React.FC<{ examen: Examen; participari: Participare[]; setParticipari: React.Dispatch<React.SetStateAction<Participare[]>>; sportivi: Sportiv[]; grade: Grad[]; onBack: () => void; preturiConfig: PretConfig[]; plati: Plata[]; setPlati: React.Dispatch<React.SetStateAction<Plata[]>>; }> = ({ examen, participari, setParticipari, sportivi, grade, onBack, preturiConfig, plati, setPlati }) => {
    ...
    const handleAddParticipant = async () => {
        ...
        const defaultGrad = grade.sort((a, b) => a.ordine - b.ordine)[0]; if (!defaultGrad) { showError("Eroare", "Nu există grade definite."); return; }
        const { data, error } = await supabase.from('participari').insert({ examen_id: examen.id, sportiv_id: sportivIdToAdd, grad_sustinut_id: defaultGrad.id, rezultat: 'Neprezentat' }).select().single();
        if (error) { showError("Eroare la adăugare", error); return; }
        setParticipari(prev => [...prev, data as Participare]); setSportivIdToAdd('');
        
        const pret = getPretGrad(defaultGrad.nume, examen.data);
        if (pret !== null) { 
            const descrierePlata = `Taxa Examen ${defaultGrad.nume} - ${examen.sesiune} ${new Date(examen.data).getFullYear()}`;
    ...
    const handleUpdateParticipare = async (id: string, updates: Partial<Participare>) => { if (!supabase) return; let finalUpdates = { ...updates }; const original = participari.find(p => p.id === id); if(!original) return; if (Object.keys(updates).some(k => k.startsWith('nota_'))) { finalUpdates.media = calculateMedia({ ...original, ...updates }); } setParticipari(prev => prev.map(p => p.id === id ? { ...p, ...finalUpdates } : p)); const { error } = await supabase.from('participari').update(finalUpdates).eq('id', id); if (error) { showError("Eroare la salvare", error); setParticipari(prev => prev.map(p => p.id === id ? original : p)); } };
    
    const handleGradeChange = async (participareId: string, newGradId: string) => {
        const participare = participari.find(p => p.id === participareId);
    ...
    const handleBulkImport = async (dataToImport: Omit<Participare, 'id'>[]) => {
        ...
        const updatedParticipari = participari.filter(p => !importedSportivIds.has(p.sportiv_id));
