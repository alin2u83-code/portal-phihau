...
export const ExameneManagement: React.FC<ExameneManagementProps> = ({ onBack, examene, setExamene, participari, setParticipari, sportivi, grade, setPlati, preturi }) => {
  ...
  return ( <div><Button onClick={onBack} variant="secondary" className="mb-6"><ArrowLeftIcon className="w-5 h-5 mr-2" /> Înapoi la Meniu</Button><div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold text-slate-900">Management Examene</h1><Button onClick={() => { setExamenToEdit(null); setIsFormOpen(true); }} variant="info"><PlusIcon className="w-5 h-5 mr-2" />Adaugă Examen</Button></div><div className="bg-white rounded-lg shadow-lg overflow-hidden"><table className="w-full text-left"><thead className="bg-slate-50 border-b border-slate-200"><tr><th className="p-4 text-sm font-semibold text-slate-600">Data</th><th className="p-4 text-sm font-semibold text-slate-600">Locația</th><th className="p-4 text-sm font-semibold text-slate-600">Participanți</th><th className="p-4 text-sm font-semibold text-slate-600">Acțiuni</th></tr></thead><tbody className="divide-y divide-slate-200">{sortedExamene.map(examen => ( <tr key={examen.id} className="hover:bg-slate-50"><td className="p-4 font-medium cursor-pointer" onClick={() => setSelectedExamen(examen)}>{examen.data}</td><td className="p-4 cursor-pointer" onClick={() => setSelectedExamen(examen)}>{examen.locatia}</td><td className="p-4">{participari.filter(p => p.examen_id === examen.id).length}</td><td className="p-4"><div className="flex items-center space-x-2"><Button onClick={() => handleEdit(examen)} variant="primary" size="sm"><EditIcon /></Button><Button onClick={() => handleDelete(examen.id)} variant="danger" size="sm"><TrashIcon /></Button></div></td></tr> ))}{sortedExamene.length === 0 && <tr><td colSpan={4}><p className="p-4 text-center text-slate-500">Niciun examen programat.</p></td></tr>}</tbody></table></div><ExamenForm isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} onSave={handleSaveExamen} examenToEdit={examenToEdit} /></div> );
};