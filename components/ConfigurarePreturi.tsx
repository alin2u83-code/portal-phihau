...
const PretForm: React.FC<PretFormProps> = ({ isOpen, onClose, onSave, pretToEdit }) => {
    ...
    React.useEffect(() => { if (pretToEdit) setFormState(pretToEdit); else setFormState({ ...emptyFormState, valabil_de_la_data: new Date().toISOString().split('T')[0] }); }, [pretToEdit, isOpen]);
