...
import { Examen, Participare, Sportiv, Grad, PretConfig, Plata, View } from '../types';
import { Button, Modal, Input, Select, Card, ConfirmationModal } from './ui';
import { PlusIcon, EditIcon, TrashIcon, ArrowLeftIcon, UsersIcon, CheckCircleIcon } from './icons';
import { supabase } from '../supabaseClient';
import { useError } from './ErrorProvider';
import { useNotification } from './NotificationProvider';
import { QuickEditModal } from './QuickEditModal';

interface ParsedRow { id: number; fullName: string; gradName: string; nota_tehnica: number | null; nota_doc_luyen: number | null; nota_song_doi: number | null; nota_thao_quyen: number | null; sportiv_id: string | null; grad_sustinut_id: string | null; status: 'ok' | 'sportiv_not_found' | 'grad_not_found' | 'invalid_row'; selected: boolean; media_generala: number | null; }
const ImportModal: React.FC<{ isOpen: boolean; onClose: () => void; onImportConfirm: (dataToImport: Omit<Participare, 'id' | 'plata_id'>[]) => Promise<void>; sportivi: Sportiv[]; grade: Grad[]; examenId: string; sportiviNeinscrisi: Sportiv[]; }> = ({ isOpen, onClose, onImportConfirm, sportivi, grade, examenId, sportiviNeinscrisi }) => {
    ...
    const handleProcessData = () => {
        ...
        const processedRows: ParsedRow[] = rows.map((row, index) => {
            ...
            return { id: index, fullName, gradName, ...p, sportiv_id: foundSportiv?.id || null, grad_sustinut_id: foundGrad?.id || null, status, selected: status === 'ok', media_generala: calculateMedia(p) } as ParsedRow;
        });
    ...
    const handleConfirm = async () => {
        const dataToImport: Omit<Participare, 'id' | 'plata_id'>[] = parsedData.filter(row => row.selected && row.status === 'ok' && row.sportiv_id && row.grad_sustinut_id).map(row => ({ examen_id: examenId, sportiv_id: row.sportiv_id!, grad_sustinut_id: row.grad_sustinut_id!, nota_tehnica: row.nota_tehnica, nota_doc_luyen: row.nota_doc_luyen, nota_song_doi: row.nota_song_doi, nota_thao_quyen: row.nota_thao_quyen, media_generala: row.media_generala, rezultat: 'Neprezentat', observatii: 'Importat automat' }));
...
const ExamenDetail: React.FC<{ examen: Examen; participari: Participare[]; setParticipari: React.Dispatch<React.SetStateAction<Participare[]>>; sportivi: Sportiv[]; setSportivi: React.Dispatch<React.SetStateAction<Sportiv[]>>; grade: Grad[]; onBack: () => void; preturiConfig: PretConfig[]; plati: Plata[]; setPlati: React.Dispatch<React.SetStateAction<Plata[]>>; examene: Examen[]; onNavigate?: (view: View, state?: any) => void; }> = ({ examen, participari: toateParticiparile, setParticipari, sportivi, setSportivi, grade, onBack, preturiConfig, plati, setPlati, examene, onNavigate }) => {
    ...
    const [missingRequiredFields, setMissingRequiredFields] = useState<Array<keyof Sportiv>>([]);
    const [isQuickEditModalOpen, setIsQuickEditModalOpen] = useState(false);
    const [sportivForQuickEdit, setSportivForQuickEdit] = useState<Sportiv | null>(null);
    ...
    useEffect(() => {
        ...
        if (!sportiv.data_nasterii) {
            setAddParticipantForm(prev => ({ ...prev, gradId: '', eligibilityMessage: 'Data nașterii este necesară pentru a calcula eligibilitatea.', isEligible: false }));
            setMissingRequiredFields(['data_nasterii']);
            return;
        }
    ...
    const handleInscriereExamen = async () => {
        ...
        if (!sportiv) { showError("Eroare", "Sportivul selectat este invalid."); return; }
        
        if (!sportiv.data_nasterii) {
            setSportivForQuickEdit(sportiv);
            setIsQuickEditModalOpen(true);
            return;
        }

        const gradSustinut = grade.find(g => g.id === gradId);
        if (!gradSustinut) { showError("Eroare", "Gradul selectat este invalid."); return; }
        ...
        if (!plataData) { showError("Eroare Necunoscută", "Factura nu a putut fi creată."); return; }

        const { data, error } = await supabase.from('participari').insert({ examen_id: examen.id, sportiv_id: sportivId, grad_sustinut_id: gradId, rezultat: 'Neprezentat', plata_id: plataData.id }).select().single();
        
        if (error) {
            showError("Eroare la înscriere", "Nu s-a putut salva participarea. Se anulează taxa generată.");
            await supabase.from('plati').delete().eq('id', plataData.id);
            return;
        }
    ...
    const handleUpdateParticipare = async (id: string, updates: Partial<Participare>) => { if (!supabase) return; let finalUpdates = { ...updates }; const original = toateParticiparile.find(p => p.id === id); if(!original) return; if (Object.keys(updates).some(k => k.startsWith('nota_'))) { finalUpdates.media_generala = calculateMedia({ ...original, ...updates }); } setParticipari(prev => prev.map(p => p.id === id ? { ...p, ...finalUpdates } : p)); const { error } = await supabase.from('participari').update(finalUpdates).eq('id', id); if (error) { showError("Eroare la salvare", error); setParticipari(prev => prev.map(p => p.id === id ? original : p)); } };
    ...
    const handleGradeChange = async (participareId: string, newGradId: string) => {
        ...
        const plataExistenta = plati.find(p => p.id === participare.plata_id);
    ...
    const handleBulkImport = async (dataToImport: Omit<Participare, 'id' | 'plata_id'>[]) => {
        if (!supabase) return;
    ...
    const handleDeleteParticipant = async () => {
        ...
        if (participantToDelete.plata_id) {
            const plata = plati.find(p => p.id === participantToDelete.plata_id);
            if (plata && plata.status === 'Neachitat') {
                const { error: plataError } = await supabase.from('plati').delete().eq('id', plata.id);
                if (plataError) { showError("Avertisment", `Participarea a fost ștearsă, dar taxa asociată nu a putut fi ștearsă: ${plataError.message}. Ștergeți manual.`); } 
                else { setPlati(prev => prev.filter(p => p.id !== plata.id)); }
            }
        }
    ...
    return ( <div> <Button onClick={onBack} variant="secondary" className="mb-6"><ArrowLeftIcon className="w-5 h-5 mr-2" /> Sesiuni</Button> <Card> <h2 className="text-3xl font-bold text-white">{`Sesiune ${examen.sesiune} ${new Date(examen.data).getFullYear()}`}</h2> <p className="text-slate-400">{new Date(examen.data).toLocaleDateString('ro-RO')} - {examen.locatia}</p> <div className="my-6 p-4 bg-slate-900/50 rounded-lg border border-slate-700 space-y-4"> <h3 className="text-lg font-semibold mb-2">Adaugă Participant</h3> <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end"> <Select label="Sportiv" value={addParticipantForm.sportivId} onChange={e => setAddParticipantForm(p => ({...p, sportivId: e.target.value}))}><option value="">Selectează...</option>{sportiviNeinscrisi.map(s => <option key={s.id} value={s.id}>{s.nume} {s.prenume}</option>)}</Select> <Select label="Grad Propus" value={addParticipantForm.gradId} onChange={e => setAddParticipantForm(p => ({...p, gradId: e.target.value}))} disabled={!addParticipantForm.sportivId}><option value="">Alege grad...</option>{grade.sort((a,b) => a.ordine - b.ordine).map(g => <option key={g.id} value={g.id}>{g.nume}</option>)}</Select> <Button onClick={handleInscriereExamen} disabled={!addParticipantForm.sportivId || !addParticipantForm.gradId}><PlusIcon className="w-5 h-5 mr-2"/>Adaugă</Button> </div> {addParticipantForm.eligibilityMessage && (<p className={`text-xs mt-2 ${addParticipantForm.isEligible ? 'text-green-400' : 'text-amber-400'}`}>{addParticipantForm.eligibilityMessage}</p>)}
            <div className="flex justify-end mt-2"><Button variant="secondary" onClick={() => setIsImportModalOpen(true)} className="hidden md:inline-flex">Importă Date</Button></div></div> <div className="flex items-center justify-between mb-4"> <h3 className="text-xl font-bold text-white flex items-center gap-2"><UsersIcon className="w-6 h-6"/> Participanți ({participari.length})</h3> <div className="flex items-center p-1 bg-slate-700 rounded-lg"> <Button size="sm" variant={viewMode === 'note' ? 'primary' : 'secondary'} onClick={() => setViewMode('note')} className={viewMode === 'note' ? 'shadow-lg' : 'bg-transparent shadow-none'}>Note</Button> <Button size="sm" variant={viewMode === 'admin' ? 'primary' : 'secondary'} onClick={() => setViewMode('admin')} className={viewMode === 'admin' ? 'shadow-lg' : 'bg-transparent shadow-none'}>Admin</Button> </div> </div> <div className="hidden md:block overflow-x-auto"> {viewMode === 'note' ? ( <table className="w-full text-left text-sm table-fixed"> <thead className="bg-slate-700/50 text-xs uppercase text-slate-400"><tr><th className="p-2 font-semibold w-12">#</th><th className="p-2 font-semibold">Nume</th><th className="p-2 font-semibold w-48">Grad Susținut</th><th className="p-2 font-semibold w-24">Tehnică</th><th className="p-2 font-semibold w-24">Doc Luyen</th><th className="p-2 font-semibold w-24">Song Doi</th><th className="p-2 font-semibold w-24">Thao Quyen</th><th className="p-2 font-semibold w-24">Media</th></tr></thead> <tbody className="divide-y divide-slate-700 text-sm">{participari.map((p, idx) => { const sportiv = sportivi.find(s => s.id === p.sportiv_id); return (<tr key={p.id} className="hover:bg-slate-700/30"> <td className="p-2">{idx+1}</td> <td className="p-2 font-bold whitespace-nowrap overflow-hidden text-ellipsis">{sportiv?.nume} {sportiv?.prenume}</td> <td className="p-2"><Select label="" value={p.grad_sustinut_id} className="text-sm" onChange={e => handleGradeChange(p.id, e.target.value)}>{grade.map(g => <option key={g.id} value={g.id}>{g.nume}</option>)}</Select></td> {['nota_tehnica', 'nota_doc_luyen', 'nota_song_doi', 'nota_thao_quyen'].map(notaKey => (<td key={notaKey} className="p-1"><Input label="" type="number" step="0.01" min="0" max="10" defaultValue={p[notaKey as keyof Participare] as number || ''} onBlur={e => handleUpdateParticipare(p.id, { [notaKey]: e.target.value ? parseFloat(e.target.value) : null })} className="text-center p-1"/></td>))} <td className="p-2 font-bold text-brand-secondary text-center">{p.media_generala?.toFixed(2)}</td> </tr>);})}</tbody></table> ) : ( <table className="w-full text-left text-sm table-fixed"> <thead className="bg-slate-700/50 text-xs uppercase text-slate-400"><tr><th className="p-2 font-semibold w-12">#</th><th className="p-2 font-semibold">Nume</th><th className="p-2 font-semibold w-48">Grad Susținut</th><th className="p-2 font-semibold w-40">Rezultat</th><th className="p-2 font-semibold w-40">Status Plată</th><th className="p-2 font-semibold">Observații</th><th className="p-2 font-semibold w-16"></th></tr></thead> <tbody className="divide-y divide-slate-700 text-sm">{participari.map((p, idx) => { const sportiv = sportivi.find(s => s.id === p.sportiv_id); const plata = plati.find(pl => pl.id === p.plata_id); const status = plata?.status || 'Neachitat'; const statusColor = status === 'Achitat' ? 'text-green-400' : status === 'Achitat Parțial' ? 'text-yellow-400' : 'text-red-400'; return (<tr key={p.id} className="hover:bg-slate-700/30"> <td className="p-2">{idx+1}</td> <td className="p-2 font-bold whitespace-nowrap overflow-hidden text-ellipsis">{sportiv?.nume} {sportiv?.prenume}</td> <td className="p-2"><Select label="" value={p.grad_sustinut_id} className="text-sm bg-transparent border-slate-700" onChange={e => handleGradeChange(p.id, e.target.value)}>{grade.map(g => <option key={g.id} value={g.id}>{g.nume}</option>)}</Select></td> <td className="p-2"><Select label="" value={p.rezultat} onChange={e => handleUpdateParticipare(p.id, { rezultat: e.target.value as any })}><option>Admis</option><option>Respins</option><option>Neprezentat</option></Select></td> <td className="p-2"><div className="flex items-center gap-2">{plata?.status === 'Achitat' && <CheckCircleIcon className="w-5 h-5 text-green-400" />}<span className={`font-bold ${statusColor}`}>{status}</span></div></td> <td className="p-1"><Input label="" defaultValue={p.observatii || ''} onBlur={e => handleUpdateParticipare(p.id, { observatii: e.target.value })} /></td> <td className="p-2 text-right"><Button variant="danger" size="sm" onClick={() => setParticipantToDelete(p)}><TrashIcon /></Button></td> </tr>);})}</tbody></table> )} </div> <div className="md:hidden grid grid-cols-1 gap-4"> {participari.map(p => { const sportiv = sportivi.find(s => s.id === p.sportiv_id); const plata = plati.find(pl => pl.id === p.plata_id); const status = plata?.status || 'Neachitat'; const statusColor = status === 'Achitat' ? 'text-green-400' : status === 'Achitat Parțial' ? 'text-yellow-400' : 'text-red-400'; return ( <Card key={p.id} className="p-4"> <div className="flex justify-between items-start"> <h4 className="font-bold text-lg">{sportiv?.nume} {sportiv?.prenume}</h4> <Button variant="danger" size="sm" onClick={() => setParticipantToDelete(p)}><TrashIcon /></Button> </div> {viewMode === 'note' ? ( <div className="grid grid-cols-2 gap-4 mt-4"> <Select label="Grad" value={p.grad_sustinut_id} onChange={e => handleGradeChange(p.id, e.target.value)}>{grade.map(g => <option key={g.id} value={g.id}>{g.nume}</option>)}</Select> <div></div> {['nota_tehnica', 'nota_doc_luyen', 'nota_song_doi', 'nota_thao_quyen'].map(notaKey => ( <Input key={notaKey} label={notaKey.replace('nota_', '').replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())} type="number" step="0.01" min="0" max="10" defaultValue={p[notaKey as keyof Participare] as number || ''} onBlur={e => handleUpdateParticipare(p.id, { [notaKey]: e.target.value ? parseFloat(e.target.value) : null })} /> ))} <div className="col-span-2 text-center pt-2"> <p className="text-sm text-slate-400">Media</p> <p className="font-bold text-brand-secondary text-2xl">{p.media_generala?.toFixed(2) ?? 'N/A'}</p> </div> </div> ) : ( <div className="space-y-4 mt-4"> <Select label="Grad Susținut" value={p.grad_sustinut_id} onChange={e => handleGradeChange(p.id, e.target.value)}>{grade.map(g => <option key={g.id} value={g.id}>{g.nume}</option>)}</Select> <Select label="Rezultat" value={p.rezultat} onChange={e => handleUpdateParticipare(p.id, { rezultat: e.target.value as any })}><option>Admis</option><option>Respins</option><option>Neprezentat</option></Select> <div><label className="block text-sm font-medium text-slate-300 mb-1">Status Plată</label><div className="flex items-center gap-2">{plata?.status === 'Achitat' && <CheckCircleIcon className="w-5 h-5 text-green-400" />}<p className={`font-bold text-lg ${statusColor}`}>{status}</p></div></div> <Input label="Observații" defaultValue={p.observatii || ''} onBlur={e => handleUpdateParticipare(p.id, { observatii: e.target.value })} /> </div> )} </Card> ) })} </div> </Card> <ConfirmationModal isOpen={!!participantToDelete} onClose={() => setParticipantToDelete(null)} onConfirm={handleDeleteParticipant} title="Confirmare Ștergere" message="Sunteți sigur că doriți să ștergeți această înregistrare și taxa asociată (dacă este neachitată)?" loading={deleteLoading}/> <ImportModal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} onImportConfirm={handleBulkImport} sportivi={sportivi} grade={grade} examenId={examen.id} sportiviNeinscrisi={sportiviNeinscrisi}/> 
    <QuickEditModal
        isOpen={isQuickEditModalOpen}
        onClose={() => setIsQuickEditModalOpen(false)}
        sportivToEdit={sportivForQuickEdit}
        fieldsToEdit={missingRequiredFields}
        onSaveSuccess={(updatedSportiv) => {
...
export const ExameneManagement: React.FC<{ onBack: () => void; examene: Examen[]; setExamene: React.Dispatch<React.SetStateAction<Examen[]>>; participari: Participare[]; setParticipari: React.Dispatch<React.SetStateAction<Participare[]>>; sportivi: Sportiv[]; setSportivi: React.Dispatch<React.SetStateAction<Sportiv[]>>; grade: Grad[]; preturiConfig: PretConfig[]; plati: Plata[]; setPlati: React.Dispatch<React.SetStateAction<Plata[]>>; onNavigate?: (view: View, state?: any) => void; navigationState?: any; }> = ({ onBack, examene, setExamene, participari, setParticipari, sportivi, setSportivi, grade, preturiConfig, plati, setPlati, onNavigate, navigationState }) => {
  ...
  useEffect(() => {
    if (navigationState?.reopenExamenId) {
        const examenToReopen = examene.find(e => e.id === navigationState.reopenExamenId);
        if (examenToReopen) {
            setSelectedExamen(examenToReopen);
        }
    }
  }, [navigationState, examene]);
  ...
  if(selectedExamen) { return <ExamenDetail examen={selectedExamen} participari={participari} setParticipari={setParticipari} sportivi={sportivi} setSportivi={setSportivi} grade={grade} onBack={() => setSelectedExamen(null)} preturiConfig={preturiConfig} plati={plati} setPlati={setPlati} examene={examene} onNavigate={onNavigate} />; }
