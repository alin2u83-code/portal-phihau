...
class ProfileService {
  ...
  /// 2. Dacă nu găsește niciunul (folosind `maybeSingle` pentru a evita erori),
  ...
  Future<Sportiv?> getActiveUserProfile() async {
    ...
    try {
      // Pas 1: Încearcă să găsească contextul primar
      ...
      if (primaryContextResponse != null && primaryContextResponse['sportiv_id'] != null) {
        // Succes: S-a găsit contextul primar
        sportivId = primaryContextResponse['sportiv_id'];
      } else {
        // Pas 2: Fallback - nu s-a găsit un context primar
        // Caută primul context disponibil pentru a preveni blocarea aplicației.
        print('Avertisment: Niciun context primar găsit. Se aplică fallback-ul.');
        final fallbackResponse = await _supabase
            .from('utilizator_roluri_multicont')
            .select('sportiv_id')
            .eq('user_id', user.id)
            .order('created_at', ascending: true) // O ordonare consistentă
            .limit(1);
      ...
      // Pas 3: Preia profilul complet al sportivului folosind ID-ul determinat
      ...
      return Sportiv.fromJson(sportivProfileResponse);
