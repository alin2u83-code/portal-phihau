...
const emptyFormState: Omit<PretConfig, 'id' | 'valabil_de_la_data'> = { categorie: 'Taxa Examen', denumire_servisciu: '', suma: 0, specificatii: {} };
...
export const ConfigurarePreturi: React.FC<ConfigurarePreturiProps> = ({ preturi, setPreturi, onBack, sportivi }) => {
    ...
    const sortedPreturi = [...preturi].sort((a, b) => {
        if (a.denumire_servisciu < b.denumire_servisciu) return -1;
        if (a.denumire_servisciu > b.denumire_servisciu) return 1;
    ...
    const echipamenteDisponibile = useMemo(() => {
        return [...new Set(preturi.filter(p => p.categorie === 'Echipament').map(p => p.denumire_servisciu))]
    }, [preturi]);
