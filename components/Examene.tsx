...
const calculateMedia = (participare: Partial<Participare>): number | null => {
    const note = [participare.nota_tehnica, participare.nota_doc_luyen, participare.nota_song_doi, participare.nota_thao_quyen].filter(n => typeof n === 'number') as number[];
        ...
        <div>
            ...
            <Card>
                ...
                <div className="my-6 p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                    <h3 className="text-lg font-semibold mb-2">Adaugă Sportiv</h3>
                           ...
                           <tbody className="divide-y divide-slate-700">
                               {participari.map((p, idx) => {
                                   ...
                                   <tr key={p.id} className="hover:bg-slate-700/30">
                                       ...
                                       <td className="p-2 text-right"><Button variant="danger" size="sm" onClick={async () => { if(window.confirm('Sigur?')) { await supabase.from('participari').delete().eq('id', p.id); setParticipari(prev => prev.filter(pa => pa.id !== p.id)); }}}><TrashIcon /></Button></td>
                                   </tr>
                                   );
                               })}
                           </tbody>
...
export const ExameneManagement: React.FC<{ onBack: () => void; examene: Examen[]; setExamene: React.Dispatch<React.SetStateAction<Examen[]>>; participari: Participare[]; setParticipari: React.Dispatch<React.SetStateAction<Participare[]>>; sportivi: Sportiv[]; grade: Grad[]; }> = ({ onBack, examene, setExamene, participari, setParticipari, sportivi, grade }) => {
  ...
  const handleDelete = async (examenId: string) => {
    if (!supabase || !window.confirm("Atenție! Veți șterge sesiunea și TOATE rezultatele asociate. Continuați?")) return;
            ...
            <tbody className="divide-y divide-slate-700">
              {sortedExamene.map(examen => ( 
                <tr key={examen.id} className="hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="p-4 text-right w-32"><div className="flex justify-end gap-2"><Button onClick={() => {setExamenToEdit(examen); setIsFormOpen(true);}} variant="primary" size="sm"><EditIcon /></Button><Button onClick={() => handleDelete(examen.id)} variant="danger" size="sm"><TrashIcon /></Button></div></td>
                </tr> 
              ))}
            </tbody>
          </table>
