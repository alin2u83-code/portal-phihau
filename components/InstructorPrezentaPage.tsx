...
import { Antrenament, Sportiv, Grupa, User, View } from '../types';
...
const AthleteRow: React.FC<AthleteRowProps> = ({ sportiv, isPresent, onToggle, isExtra, onRemove }) => (
    <div className={`flex items-center gap-3 p-2 rounded-md transition-all duration-200 ${isPresent ? 'bg-green-900/40' : 'bg-slate-800/50 opacity-60 hover:opacity-100'}`}>
        <input
            id={`att-${sportiv.id}`}
            type="checkbox"
...
const TrainingCard: React.FC<TrainingCardProps> = ({ training, allClubSportivi, initialPresentIds, onSave }) => {
    ...
    return (
        <Card className="flex flex-col">
            ...
            <div className="space-y-2 flex-grow max-h-96 overflow-y-auto pr-2">
                {groupAthletes.map(s => <AthleteRow key={s.id} sportiv={s} isPresent={presentIds.has(s.id)} onToggle={handleToggle} isExtra={false} />)}
                {extraAthletes.length > 0 && (
                    <div className="pt-2 mt-2 border-t border-slate-700">
                        <h4 className="text-xs font-bold text-slate-400 uppercase mb-2">Participanți Externi</h4>
                        {extraAthletes.map(s => <AthleteRow key={s.id} sportiv={s} isPresent={presentIds.has(s.id)} onToggle={handleToggle} isExtra={true} onRemove={handleRemoveExtra} />)}
                    </div>
                )}
            </div>
    ...
    useEffect(() => {
        const fetchTodaysTrainings = async () => {
            ...
            const { data, error } = await supabase
                ...
                .select('*, grupe(*, sportivi(id, nume, prenume, status)), prezenta:prezenta_antrenament(sportiv_id)')
