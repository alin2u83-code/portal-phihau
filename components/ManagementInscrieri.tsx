...
import { SesiuneExamen, Sportiv, InscriereExamen, Grad, PretConfig, Plata } from '../types';
...
export const ManagementInscrieri: React.FC<ManagementInscrieriProps> = ({ sesiune, sportivi, allInscrieri, grade, preturiConfig, setInscrieri, setPlati }) => {
    ...
    const sportiviData = useMemo(() => {
        return sportivi
            ...
            .map(s => {
                const varstaLaExamen = getAgeOnDate(s.data_nasterii, sesiune.data);
                const admittedInscrieri = allInscrieri
                    .filter(i => i.sportiv_id === s.id && i.rezultat === 'Admis')
                    .sort((a, b) => (getGrad(b.grad_vizat_id, grade)?.ordine ?? 0) - (getGrad(a.grad_vizat_id, grade)?.ordine ?? 0));
                
                const gradActual = getGrad(admittedInscrieri[0]?.grad_vizat_id, grade);

                let gradVizat: Grad | undefined;
                if (!gradActual) {
                    if (varstaLaExamen >= 5 && varstaLaExamen <= 6) gradVizat = sortedGrades.find(g => g.nume.toLowerCase().includes('galben'));
                    else if (varstaLaExamen >= 7 && varstaLaExamen <= 12) gradVizat = sortedGrades.find(g => g.nume.toLowerCase().includes('roș'));
                    else gradVizat = sortedGrades.find(g => g.nume.toLowerCase().includes('albastr'));
                } else {
    ...
    }, [sportivi, searchTerm, allInscrieri, grade, sesiune, sortedGrades, inscrisiInSesiuneIds]);
    ...
    const handleInscriere = async (sportiv: Sportiv, gradVizat: Grad | undefined) => {
        ...
        const gradActual = sportiviData.find(d => d.sportiv.id === sportiv.id)?.gradActual;
        const varstaLaExamen = getAgeOnDate(sportiv.data_nasterii, sesiune.data);
        const taxa = getPretProdus(preturiConfig, 'Taxa Examen', gradVizat.nume, { dataReferinta: sesiune.data })?.suma;

        try {
            const newInscriere = { sportiv_id: sportiv.id, sesiune_id: sesiune.id, grad_actual_id: gradActual?.id || null, grad_vizat_id: gradVizat.id, varsta_la_examen: varstaLaExamen, rezultat: 'Neprezentat' as const };
            const { data: iData, error: iError } = await supabase.from('inscrieri_examene').insert(newInscriere).select('*, sportivi:sportiv_id(*), grade:grad_vizat_id(*)').single();
            if (iError) throw iError;
            
            setInscrieri(prev => [...prev, iData as InscriereExamen]);

            if (taxa) {
                const newPlata = { sportiv_id: sportiv.id, familie_id: sportiv.familie_id, suma: taxa, data: sesiune.data, status: 'Neachitat' as const, descriere: `Taxa examen grad ${gradVizat.nume}`, tip: 'Taxa Examen' as const, observatii: 'Generat automat la înscriere.' };
