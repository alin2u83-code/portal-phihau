...
- Faza 30 (2026-09-20, toate 6 planuri executate) — 4 blockere ramase pt sesiunea urmatoare:
  1. `sms_config` si `sms_templates` complet goale pe toate cluburile (0 randuri) — mecanismul de memento SMS (30-01/30-03) e corect reparat dar mut: fara un rand `sms_config` (gateway) si un `sms_templates` activ per club, nu pleaca niciun mesaj indiferent de configurare praguri.
  2. RLS pe `politici_reducere` are o singura politica (`Bypass_Super_Admin`) — ADMIN_CLUB nu poate citi/scrie politici de loialitate deloc (nu doar scoping per club, acces zero). `getPoliticiLoialitate()` din 30-05 returneaza mereu `[]` in context ADMIN_CLUB. Decizie de business necesara: extindere RLS (`has_access_to_club`) sau ramane doar federatie.
  3. `sms-process-queue` NEACTIVAT — checkpoint uman explicit "amanare" (30-06). Blocaj tehnic separat: nici `current_setting('app.service_role_key')`, nici `vault.decrypted_secrets` nu au cheia service_role pt `net.http_post`; cineva cu acces Supabase Dashboard trebuie sa puna manual secretul (`vault.create_secret` sau `ALTER DATABASE ... SET app.service_role_key`) inainte de a rula `cron.schedule('sms-process-queue', ...)` din docs/sms-system/ANDROID_GATEWAY_SETUP.md § Pasul 7.
  4. Cazurile "pozitive" netestate live: reset efectiv `data_start_facturare` (gratie) pe un sportiv cu pauza reala >30 zile, si bonus de loialitate aplicat pe o factura reala (blocat de blocker #2 pana la extindere RLS). Testat live doar cazul negativ (nimic de resetat/bonusat) — vezi 30-05-SUMMARY.md.
